...
panel.port.on('refresh', () => {
  browser.src = 'https://www.messenger.com/';
  browser.reload();
});
...
panel.port.on('init', () => {
  let panelView = getActiveView(panel);
    ...
    // messenger.com cannot be loaded in an iframe; we use a safe browser element (type=content)
    ...
    browser.setAttribute('src', 'https://www.messenger.com/');
    ...
    if (self.version !== version) {
      if (sp.prefs.welcome) {
        timers.setTimeout(function () {
          tabs.open(
            'http://add0n.com/messenger.html?v=' + self.version +
